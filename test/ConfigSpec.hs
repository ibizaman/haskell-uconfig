...
spec = do
    H.describe "spaced" $ do
        H.it "parses without space"
            ...
            `H.shouldBe` Right (Spaced 0 0 "word")
        H.it "parses with space"
            $            parse (spaced P.word) "  word "
            `H.shouldBe` Right (Spaced 2 1 "word")
        H.it "parses with trailing space"
            $            parse (spaced P.word) "word "
            `H.shouldBe` Right (Spaced 0 1 "word")
    H.describe "quoted" $ do
        H.it "not quoted" $ parse (quoted P.word) "word" `H.shouldBe` Right
            (NotQuoted "word")
        H.it "double quoted"
        ...
        H.it "single quoted" $ parse (quoted P.word) "'word'" `H.shouldBe` Right
            (SingleQuoted 0 0 "word")
    ...
    H.describe "anyHeader" $ do
        H.it "parses header" $ parse anyHeader "[section]" `H.shouldBe` Right
            "section"
